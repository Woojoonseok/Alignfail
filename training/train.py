...
def exclusive_lock(path):
    ...
    try:
        ...
        handle.close()
        raise RuntimeError("Another training process holds the device lock") from None
    return handle
...
def run(directory, managed=False):
    ...
    try:
        ...
        if config["device"] == "cuda" and not torch.cuda.is_available():
            raise RuntimeError(
                "CUDA unavailable. Set ALIGNFAIL_TRAINING_PYTHON to the WSL CUDA environment, or choose CPU for a smoke test."
            )
        seed = config["seed"]
        random.seed(seed)
        np.random.seed(seed)
        ...
        env.update(
            ...
            gpu=torch.cuda.get_device_name(0) if config["device"] == "cuda" else "CPU",
            numpy=np.__version__,
            opencv=cv2.__version__,
            model="metric_patch_v1: shared CNN + GAP, dense context pooling; no legacy weights",
            padding="reflect101",
        ...
        for epoch in range(1, config["epochs"] + 1):
            ...
            if scores["Overall"]["median_error"] < best:
                ...
                torch.save(checkpoint, directory / "best.tmp")
                (directory / "best.tmp").replace(directory / "best.pt")
