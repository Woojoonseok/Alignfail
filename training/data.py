...
def crop_spec(box, mode, config):
    ...
    size = int(
        np.floor(
            np.clip(
                max(box[2] - box[0], box[3] - box[1]) * config["context_ratio"], config["min_crop"], config["max_crop"]
            )
            + 0.5
        )
    )
...
def crop(image, center, native_size, output_size):
    ...
    result = (
        native
        if output_size == native_size
        else cv2.resize(native, (output_size, output_size), interpolation=cv2.INTER_LINEAR)
    )
    return result, {
        ...
        "resize_scale": output_size / native_size,
...
def diagnostics(image, config):
    ...
    flags = {
        "near_black": mean < config["near_black_mean"],
        "low_std": std < config["low_std"],
        "low_edge_density": edge < config["low_edge_density"],
    }
    return {
        "mean": mean,
        "std": std,
...
def group_split(pairs, config):
    ...
    split = {
        ...
        "validation": [p["pair_id"] for p in val],
        "train_groups": sorted({p["group_key"] for p in train}),
