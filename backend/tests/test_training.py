...
def test_adaptive_clamp_rounding_and_center():
    ...
    assert np.isclose(patch.mean(),300.25,atol=.01)
...
def test_group_split_disjoint_reproducible_and_duplicate_hash_block():
    ...
    rows=[{"pair_id":str(i),"group_key":str(i//2),**{k:str(i) for k in ["ref_hash","query_hash","ref_original_hash","query_original_hash"]}} for i in range(8)]
...
def test_evaluation_zero_and_three_four_five():
    rows=[{"error":float(np.linalg.norm(np.subtract(a,b))),"pattern_type":p} for a,b,p in [([2,3],[2,3],"A"),([3,4],[0,0],"B")]]
    scores=metrics(rows)
...
def training_client(tmp_path):
    ...
    for i in range(4):
        folder=root/f"pair_{i}";folder.mkdir(parents=True)
    ...
    with TestClient(app) as client:
        ...
        for i,p in enumerate(pairs):
            response=client.put(f"/api/pairs/{p['id']}",json={"revision":p["revision"],"gt_x":100,"gt_y":85,"group_key":f"capture-{i}","pattern_type":"A" if i%2 else "B"})
            p=response.json()
            response=client.put(f"/api/pairs/{p['id']}/reference-annotation",json={"revision":p["revision"],"image_hash":p["reference"]["file_hash"],"box":[60,45,140,125]})
...
def test_manifest_frozen_after_live_edit_and_new_version(training_client):
    ...
    exp=result.json();path=root/exp["id"]
    before=(path/"dataset_manifest.json").read_bytes()
    pair=client.get(f"/api/projects/{pid}/pairs").json()[0]
    client.put(f"/api/pairs/{pair['id']}",json={"revision":pair["revision"],"gt_x":90,"gt_y":80,"group_key":"new-group","pattern_type":"B"})
...
def test_training_blocks_missing_roi_and_nonmanual_supervision(training_client,tmp_path,invalid):
    ...
    with pytest.raises(ValueError,match="ROI|수동 Query GT"):
        prepare(SimpleNamespace(manifest=snapshot,id=version["id"],number=1,project_id=pid),TrainingConfig(folds=2).model_dump(),tmp_path/"rejected")
...
def test_real_cpu_training_each_crop_mode(training_client,monkeypatch,mode):
    ...
    result=client.post("/api/training/prepare",json={"version_id":version["id"],"config":{"folds":2,"device":"cpu","epochs":1,"embedding_dim":16,"crop_mode":mode}})
    assert result.status_code==201,result.text
    ...
    assert client.get(f"/api/experiments/{experiment}/files/heatmaps/{finished['predictions'][0]['pair_id']}.png").status_code==200
