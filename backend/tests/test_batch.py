...
def test_auto_cleanup_no_candidate_does_not_change_revision(cleanup_client):
    ...
    result = client.post(
        f"/api/images/{image['id']}/clean/auto", json={"source_hash": image["file_hash"], "cross": False}
    )
...
def test_bulk_groups_are_atomic_scoped_and_clearable(batch_client):
    ...
    assert (
        client.post(
            path, json={"assignments": [{**a, "revision": a["revision"] + 1, "group_key": ""} for a in assignments]}
        ).json()["updated"]
        == 4
    )
...
def test_cluster_uses_clean_and_skips_tampered_cache(batch_client):
    ...
    saved = client.post(
        ...
        json={"source_hash": image["file_hash"], "box": {"x0": 10, "y0": 10, "x1": 40, "y1": 40}},
    )
...
def test_class_labels_are_preserved_in_versions_and_diff(batch_client):
    ...
    assert (
        client.post(
            f"/api/projects/{pid}/groups/bulk",
            json={"assignments": [{"id": p["id"], "revision": p["revision"], "class_label": "after"} for p in pairs]},
        ).status_code
        == 200
    )
    second = client.post(f"/api/projects/{pid}/versions", json={"description": "after"}).json()
    assert all(
        p["class_label"] == "before" for p in client.get(f"/api/versions/{first['id']}/manifest").json()["pairs"]
    )
    assert all(
        p["class_label"] == "after" for p in client.get(f"/api/versions/{second['id']}/manifest").json()["pairs"]
    )
